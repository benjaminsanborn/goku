...
> `goku.yaml` declares *what the project needs*. The platform materializes it as AWS resources; the local workspace materializes it as **local cognates**. Application code sees the identical env-var contract in both, so it never knows which plane it's running on.

```mermaid
flowchart LR
    subgraph local["Local workspace (git clone)"]
        code["api/ · web/ · goku.yaml"]
        subgraph cognates["Local cognates (docker, managed by CLI)"]
            lpg[("postgres:16")]
            lminio[("minio (S3 API)")]
            lrouter["local router<br/>*.localhost"]
        end
        code -->|"DATABASE_URL, STORAGE_URL<br/>(injected by goku dev)"| cognates
    end
    ...
    local -->|git push goku <branch>| repo --> cs -->|merge| aws
...
```
hello-world/
├── goku.yaml          # the manifest — single source of truth for both planes
├── api/
│   ├── Dockerfile
│   └── ...code
├── web/
...
└── .goku/             # gitignored: compose file, injected env, local state
```
...
1. **The env contract is the interface.** In prod, the platform injects `DATABASE_URL`, `STORAGE_URL`/`STORAGE_BUCKET`, etc. from provisioned resources. Locally, `goku dev` injects the same names pointing at cognates. Application code contains zero environment detection.
...
```
goku new hello-world     # create remote project (API), clone it, scaffold goku.yaml
goku clone hello-world   # existing project → local workspace
goku add database main   # edit manifest + start cognate + inject env (one step)
goku dev                 # read manifest → up cognates (compose) → run services with injected env
goku env                 # print the injected contract (for debugging)
goku push                # push branch + open changeset (wraps git push + MCP)
goku status              # remote state: changesets, deploys, drift
```

`goku add` is the answer to "if a design calls for a DB": it edits `goku.yaml` *and* starts the local cognate in one motion, so the manifest and the local environment cannot disagree. The manifest diff then rides in the changeset, and merge materializes the real Aurora. One decision, declared once, realized twice.

Cognates run via a generated docker-compose file in `.goku/` — per-project isolation (two projects = two postgres containers on different ports), pinned versions, disposable state. `goku dev --reset` wipes local data. Docker is a hard dependency of local dev; that's an acceptable, conventional ask.
...
- `.goku/` (local state, env, credentials) is gitignored by scaffold; the push path can also hard-reject files matching secret patterns as a belt-and-suspenders control.

## Flow: "add a feature that needs a database"

1. Human to local Claude: *"add persistent todos."*
2. Claude (shell): `goku add database main` → manifest gains the resource, `postgres:16` cognate starts, `DATABASE_URL` now injected locally.
3. Claude writes the migration + code, runs `goku dev`, tests against the cognate.
4. `git push goku claude/todos` + `open_changeset` → changelog shows code diff **and** manifest plan ("creates Aurora Serverless v2, ~$43/mo").
