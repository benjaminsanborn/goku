...
Auth model (dev slice): one token guards everything; `X-Goku-Actor: operator` distinguishes the UI from agents in the audit trail. Real token→identity mapping is designed in [docs/design/06](../docs/design/06-security-compliance.md).
