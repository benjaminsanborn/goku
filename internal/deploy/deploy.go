...
func ParseManifest(raw string) (*Manifest, error) {
	...
	return &Manifest{Services: doc.Services, Resources: doc.Resources}, nil
}

// Port allocates a stable host port for a project's app container.
func Port(project string) int {
	h := fnv.New32a()
	h.Write([]byte("app/" + project))
	return 30000 + int(h.Sum32()%10000)
}
...
func Run(project, image string, port int, env map[string]string, logf Logf) (string, error) {
